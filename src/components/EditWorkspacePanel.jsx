...
const EditWorkspacePanel = ({ workspace, onSave, onCancel, onDelete }) => {
  ...
  const handleUpdate = async () => {
    ...
    setIsSaving(true);
    try {
      const response = await fetch(`${import.meta.env.VITE_API_URL}/api/workspaces/${workspace.id}`, {
        method: 'PUT',
      ...
      <div className="input-group">
        ...
        <div style={{display: 'flex', gap: '1rem', alignItems: 'center'}}>
          ...
          <input 
            ...
            onChange={(e) => setMinScore(e.target.value)} 
            className="styled-input"
            style={{width: '70px', padding: '0.5rem', textAlign: 'center', color: 'var(--accent-primary)', fontWeight: 'bold'}}
          />
        </div>
      </div>
